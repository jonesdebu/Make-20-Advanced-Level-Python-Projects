...
def validate_login():
    ...
    if not email_text or not pass_text:
        f2.pack()
        label4.grid()
        label4["text"] = "Fill al the fields"
        btn2.grid_remove()
        root.after(10, root.grid)
        return False
    else:
...
label1.grid(row=0, columnspan=3, pady=3, padx=10)
...
btn1 = Button(f1, text="Login", width=10, bg="white", fg="black", command=lambda: login())
...
label6 = Label(f3, text="To").grid(row=0, sticky=E, pady=5)
...
label9 = Label(f3, width=10, fg="white", bg="black", font=("Calibri 18 bold"))
